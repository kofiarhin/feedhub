...
import { selectCartItems, selectCartStoreId, selectCartTotal } from '../../features/cart/cartSelectors';
...
const CheckoutPage = () => {
  ...
  const submit = async (event) => {
    ...
    navigate(authUser ? '/orders' : '/checkout/guest-confirmation');
  };
  ...
  return (
    <section>
      ...
      <form className="app-stack" onSubmit={submit}>
        ...
        <button type="submit" disabled={createOrder.isPending}>{createOrder.isPending ? 'Placing...' : 'Place order'}</button>
      </form>
    </section>
  );
};
